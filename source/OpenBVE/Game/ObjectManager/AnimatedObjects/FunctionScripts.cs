				...
				switch (Function.InstructionSet[i]) {
						...
						if (Train != null) {
							...
							for (int j = 0; j < Train.Cars.Length; j++) {
								if (Train.Cars[j].Engine.ProvidesPower) {
									...
									if (Train.Cars[j].Engine.CurrentAcceleration < 0.0) {
										Function.Stack[s] = Train.Cars[j].Engine.CurrentAcceleration * Math.Sign(Train.Cars[j].CurrentSpeed);
									} else if (Train.Cars[j].Engine.CurrentAcceleration > 0.0) {
										Function.Stack[s] = Train.Cars[j].Engine.CurrentAcceleration * (double)Train.Handles.Reverser.Actual;
									} else {
										Function.Stack[s] = 0.0;
									}
									break;
					...
					case Instructions.TrainAccelerationMotorOfCar:
						if (Train != null) {
							...
							if (j >= 0 & j < Train.Cars.Length) {
								// hack: MotorAcceleration does not distinguish between forward/backward
								if (Train.Cars[j].Engine.CurrentAcceleration < 0.0) {
									Function.Stack[s - 1] = Train.Cars[j].Engine.CurrentAcceleration * Math.Sign(Train.Cars[j].CurrentSpeed);
								} else if (Train.Cars[j].Engine.CurrentAcceleration > 0.0) {
									Function.Stack[s - 1] = Train.Cars[j].Engine.CurrentAcceleration * (double)Train.Handles.Reverser.Actual;
								} else {
									Function.Stack[s - 1] = 0.0;
								}
							} else {
					...
					case Instructions.EngineRunningCar:
						if (Train != null)
						{
							...
							if (j >= 0 & j < Train.Cars.Length)
							{
								if (Train.Cars[j].Engine != null && Train.Cars[j].Engine.IsRunning)
								{
									Function.Stack[s - 1] = 1.0;
								}
								else
					...
					case Instructions.EngineRPM:
					{
						if (Train != null)
						{
							...
							for (int k = 0; k < Train.Cars.Length; k++)
							{
								if (Train.Cars[k].Engine is DieselEngine dieselEngine)
								{
									numEngines++;
					...
					case Instructions.EngineRPMCar:
						if (Train != null)
						{
							...
							if (j >= 0 & j < Train.Cars.Length)
							{
								if (Train.Cars[j].Engine is DieselEngine dieselEngine)
								{
