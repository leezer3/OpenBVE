...
namespace OpenBve {
	internal static partial class TrainDatParser {
		...
		internal static void ParseTrainData(string FileName, System.Text.Encoding Encoding, TrainManager.Train Train) {
			...
			{
				Train.Couplers[i] = new TrainManager.Coupler(0.9 * DistanceBetweenTheCars, 1.1 * DistanceBetweenTheCars, Train.Cars[i / 2], Train.Cars[(i / 2) + 1]);
			...
			for (int i = 0; i < Cars; i++) {
				...
				if (i != 0)
				{
					Train.Couplers[i -1].ChangeSection(-1);
				}
