	...
	class OpenBVEGame: GameWindow
	{
		...
		protected override void OnResize(EventArgs e)
		{
			if(Width == 0 && Height == 0)
			{
				 ...
				 * stuff.....
				 */
				OpenBveShared.Renderer.Minimized = true;
				return;
			}
			OpenBveShared.Renderer.Minimized = false;
		...
		private void SetupSimulation()
		{
			...
			if (TrainManager.PlayerTrain.Station >= 0)
			{
				...
				if (OpenBveShared.Timetable.CustomObjectsUsed != 0 & OpenBveShared.Timetable.CustomTimetableAvailable && Interface.CurrentOptions.TimeTableStyle != Interface.TimeTableMode.AutoGenerated && Interface.CurrentOptions.TimeTableStyle != Interface.TimeTableMode.None)
				{
					OpenBveShared.Timetable.CurrentTimetable = TimetableState.Custom;
				}
			}
		...
		private void LoadingScreenLoop()
		{
			...
			while (!Loading.Complete && !Loading.Cancel)
			{
				...
				if (this.IsExiting)
					Loading.Cancel = true;
				OpenBveShared.Renderer.DrawLoadingScreen(OpenBveShared.Renderer.Width, OpenBveShared.Renderer.Height, Loading.RouteProgress, Loading.TrainProgress);
				Program.currentGameWindow.SwapBuffers();
