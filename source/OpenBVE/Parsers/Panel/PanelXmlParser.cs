			...
			if (GroupIndex == 0)
			{
					...
					switch (Section.ToLowerInvariant())
					{
						case "this":
							foreach (XElement KeyNode in SectionElement.Elements())
							{
								...
								{
									case "resolution":
										double pr = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										{
										...
										break;
									case "left":
										PanelLeft = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "right":
										PanelRight = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										...
										break;
									case "top":
										PanelTop = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "bottom":
										PanelBottom = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "daytimeimage":
				...
				if (PanelDaytimeImage != null)
				{
					...
					else
					{
						...
						Panel2CfgParser.CreateElement(ref CarSection.Groups[GroupIndex], Vector2.Null, tday.Size, new Vector2(0.5, 0.5), OffsetLayer * StackDistance, PanelResolution, PanelTop, PanelBottom, PanelCenter, Train.Cars[Car].Driver, tday, tnight, Color32.White, false);
					}
				}
			}
					...
					case "pilotlamp":
						{
							...
							if (DaytimeImage != null)
							{
								...
								int j = Panel2CfgParser.CreateElement(ref CarSection.Groups[GroupIndex], Location, tday.Size, new Vector2(0.5, 0.5), (OffsetLayer + Layer) * StackDistance, PanelResolution, PanelTop, PanelBottom, PanelCenter, Train.Cars[Car].Driver, tday, tnight, Color32.White, false);
					...
					case "needle":
						{
							...
							Color24 TransparentColor = Color24.Blue;
							Vector2 Origin = new Vector2(-1.0, -1.0);
							...
							foreach (XElement KeyNode in SectionElement.Elements())
							{
								...
								switch (Key.ToLowerInvariant())
								{
									...
									case "location":
										...
										break;
									case "radius":
										Radius = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										if (Radius == 0.0)
										{
									...
									case "initialangle":
										...
										break;
									case "lastangle":
										LastAngle = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "minimum":
										Minimum = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "maximum":
										Maximum = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "naturalfreq":
										NaturalFrequency = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										...
										break;
									case "dampingratio":
										DampingRatio = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										if (DampingRatio < 0.0)
										{
											Interface.AddMessage(MessageType.Error, false, "Value is expected to be non-negative in " + Key + " in " + Section + " at line " + LineNumber.ToString(Culture) + " in " + FileName);
										...
										break;
									case "layer":
										Layer = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
									case "backstop":
							...
							if (DaytimeImage != null)
							{
								...
								double ox = Origin.X / w;
								double oy = Origin.Y / h;
								double n = Radius == 0.0 | Origin.Y == 0.0 ? 1.0 : Radius / Origin.Y;
								double nx = n * w;
								double ny = n * h;
								int j = Panel2CfgParser.CreateElement(ref CarSection.Groups[GroupIndex], new Vector2(Location.X - ox * nx, Location.Y - oy * ny), new Vector2(nx, ny), new Vector2(ox, oy), (OffsetLayer + Layer) * StackDistance, PanelResolution, PanelTop, PanelBottom, PanelCenter, Train.Cars[Car].Driver, tday, tnight, Color, false);
					...
					case "digitalnumber":
						{
							...
							if (DaytimeImage != null & Interval > 0)
							{
								...
								if (wday > 0 & hday > 0)
								{
									...
									{
										int l = Panel2CfgParser.CreateElement(ref CarSection.Groups[GroupIndex], Location, new Vector2(wday, Interval), new Vector2(0.5, 0.5), (OffsetLayer + Layer) * StackDistance, PanelResolution, PanelTop, PanelBottom, PanelCenter, Train.Cars[Car].Driver, tday[k], tnight[k], Color32.White, k != 0);
										if (k == 0) j = l;
									}
									string f = Panel2CfgParser.GetStackLanguageFromSubject(Train, Subject, Section + " in " + FileName);
					...
					case "digitalgauge":
						{
							...
							foreach (XElement KeyNode in SectionElement.Elements())
							{
								...
								switch (Key.ToLowerInvariant())
								{
									...
									case "color":
										...
										break;
									case "initialangle":
										InitialAngle = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName).ToRadians();
										break;
									case "lastangle":
										LastAngle = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName).ToRadians();
										break;
									case "minimum":
										Minimum = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										break;
							...
							{
								// create element
								int j = Panel2CfgParser.CreateElement(ref CarSection.Groups[GroupIndex], Location - Radius, new Vector2(2.0 * Radius, 2.0 * Radius), new Vector2(0.5, 0.5), (OffsetLayer + Layer) * StackDistance, PanelResolution, PanelTop, PanelBottom, PanelCenter, Train.Cars[Car].Driver, null, null, Color, false);
					...
					case "timetable":
						{
							...
							foreach (XElement KeyNode in SectionElement.Elements())
							{
								...
								switch (Key.ToLowerInvariant())
								{
									case "location":
										...
										break;
									case "width":
										Width = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										if (Width <= 0.0)
										{
										...
										break;
									case "height":
										Height = NumberFormats.ParseDouble(Value, Key, Section, LineNumber, FileName);
										if (Height <= 0.0)
										{
											Interface.AddMessage(MessageType.Error, false, "ValueInPixels is required to be positive in " + Key + " in " + Section + " at line " + LineNumber.ToString(Culture) + " in " + FileName);
										}
							...
							{
								int j = Panel2CfgParser.CreateElement(ref CarSection.Groups[GroupIndex], Location, new Vector2(Width, Height), new Vector2(0.5, 0.5), (OffsetLayer + Layer) * StackDistance, PanelResolution, PanelTop, PanelBottom, PanelCenter, Train.Cars[Car].Driver, null, null, Color32.White, false);
