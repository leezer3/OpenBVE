	...
	public class GLTabContainer : GLContainer
	{
		...
		public override void Draw()
		{
			...
			for (int i = 0; i < TabTitles.Count; i++)
			{
				...
				Color128 headerBg = isSelected ? new Color128(0.2f, 0.22f, 0.25f, 0.9f) : InactiveTabColor;
			...
			for (int i = 0; i < TabContents.Count; i++)
			{
				...
				if (isSelected)
				{
					...
					TabContents[i].Location = new Vector2(Location.X, Location.Y + TabHeaderHeight);
					TabContents[i].Size = new Vector2(Size.X, Size.Y - TabHeaderHeight);
					TabContents[i].Draw();
				}
			}
		}
