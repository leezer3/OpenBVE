...
namespace OpenBve
{
	class TrackFollowingObjectParser
	{
		...
		private static void ParseTrackFollowingObjectNode(XElement Element, string FileName, string Path, string ObjectPath, TrainManager.TrackFollowingObject Train)
		{
				...
				switch (SectionElement.Name.LocalName.ToLowerInvariant())
				{
								...
								case "directory":
									{
										...
										if(!System.IO.Directory.Exists(trainDirectory))
										{
											Program.CurrentHost.AddMessage(MessageType.Error, false, "Directory was not found in " + Key + " in " + Section + " at line " + LineNumber.ToString(Culture) + " in " + FileName);
										}
										else
										{
					...
					case "definition":
						foreach (XElement KeyNode in SectionElement.Elements())
						{
							...
							{
								case "appearancetime":
									if (Value.Length != 0 && !CsvRwRouteParser.TryParseTime(Value, out Train.AppearanceTime))
									{
										Program.CurrentHost.AddMessage(MessageType.Error, false, "Value is invalid in " + Key + " in " + Section + " at line " + LineNumber.ToString(Culture) + " in " + FileName);
									}
									break;
								...
								case "appearanceendposition":
									...
									break;
								case "leavetime":
									if (Value.Length != 0 && !CsvRwRouteParser.TryParseTime(Value, out Train.LeaveTime))
									{
										Program.CurrentHost.AddMessage(MessageType.Error, false, "Value is invalid in " + Key + " in " + Section + " at line " + LineNumber.ToString(Culture) + " in " + FileName);
									}
									break;
			...
			{
				Program.CurrentHost.AddMessage(MessageType.Error, true, "The supplied train folder in TrackFollowingObject " + FileName + " did not contain a complete set of data.");
				return;
			}
							...
							foreach (XElement KeyNode in SectionElement.Elements())
							{
								...
								switch (Key.ToLowerInvariant())
								{
									...
									case "stopposition":
										...
										break;
									case "stoptime":
										if (Value.Length != 0 && !CsvRwRouteParser.TryParseTime(Value, out NewData.StopTime))
										{
											Program.CurrentHost.AddMessage(MessageType.Error, false, "Value is invalid in " + Key + " in " + Section + " at line " + LineNumber.ToString(Culture) + " in " + FileName);
										}
										break;
