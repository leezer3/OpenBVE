...
namespace OpenBve
{
	public partial class Menu
	{
		...
		private static void routeWorkerThread_doWork(object sender, DoWorkEventArgs e)
		{
			...
			routeDescriptionBox.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "start_route_processing");
			Game.Reset(false);
			bool loaded = false;
		...
		{
			routeDescriptionBox.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "packages_creation_failure_error") + Environment.NewLine;
			if (e.Exception is UnauthorizedAccessException && currentOperation != PackageOperation.Creating)
			{
				...
				routeDescriptionBox.Text += e.Exception.Message + Environment.NewLine + Environment.NewLine + Translations.GetInterfaceString(HostApplication.OpenBve, "errors_security_checkaccess");
				if (Program.CurrentHost.Platform == HostPlatform.MicrosoftWindows)
				{
					routeDescriptionBox.Text += Environment.NewLine + Environment.NewLine + Translations.GetInterfaceString(HostApplication.OpenBve, "errors_security_badlocation");
				}
			}
			else
			{
		...
		private static void OnPackageOperationCompleted(object sender, CompletionReport e)
		{
			...
			{
				case PackageOperation.Installing:
					routeDescriptionBox.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "packages_install_success") + Environment.NewLine + Environment.NewLine + Translations.GetInterfaceString(HostApplication.OpenBve, "packages_install_success_files") + Environment.NewLine + installedFiles;
