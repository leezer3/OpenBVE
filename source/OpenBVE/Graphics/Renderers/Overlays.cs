		...
		internal void Render(double TimeElapsed)
		{
			...
			switch (Program.Renderer.CurrentInterface)
			{
				case InterfaceType.Pause:
				{
					...
					renderer.OpenGlString.Draw(renderer.Fonts.VeryLargeFont, Translations.GetInterfaceString(HostApplication.OpenBve, "menu_pause_title"), new Vector2(renderer.Screen.Width / 2.0, renderer.Screen.Height / 2.0), TextAlignment.CenterMiddle, Color128.White, true);
					if (!PauseAnnounced && Interface.CurrentOptions.ScreenReaderAvailable)
					{
						if (!Tolk.Output(Translations.GetInterfaceString(HostApplication.OpenBve, "menu_pause_title")))
						{
