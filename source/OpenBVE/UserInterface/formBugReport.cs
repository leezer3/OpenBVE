...
namespace OpenBve
{
	internal partial class formBugReport : Form
	{
		...
		private void ApplyLanguage() {
			this.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_title");
			textBoxReportLabel.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_description");
			labelViewLog.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_view_log");
			labelViewCrash.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_view_crash_log");
			label1.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_enter_description");
			buttonReportProblem.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_save");
			buttonViewLog.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_view_log_button");
			buttonViewCrashLog.Text = Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_view_log_button");
		}
		...
		private void buttonViewLog_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if(File.Exists(file))
				{
					...
					MessageBox.Show(Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_no_log"), Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_title"), MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
			}
			catch
			{
		...
		private void buttonViewCrashLog_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show(Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_no_crash_log"), Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_title"), MessageBoxButtons.OK, MessageBoxIcon.Information);
			}
		}
			...
			try
			{
				using (var ProblemReport = File.OpenWrite(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), fileName)))
				{
					using (var zipWriter = WriterFactory.Open(ProblemReport, ArchiveType.Zip, CompressionType.LZMA))
					{
						...
						MessageBox.Show(Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_saved").Replace("[filename]", fileName), Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_title"), MessageBoxButtons.OK, MessageBoxIcon.Information);
					}
				}
			}
			catch
			{
				MessageBox.Show(Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_save_failed"), Translations.GetInterfaceString(HostApplication.OpenBve, "bug_report_title"), MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
