...
namespace SoundEditor
{
	public partial class FormEditor : Form
	{
		...
		private SoundCfg.Sounds soundCfg;
		private bool isSoundXml;
		...
		public FormEditor()
		{
			...
			Interface.LogMessages.CollectionChanged += LogMessagesChanged;
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			...
			radioButtonOpenCfg.Checked = radioButtonSaveCfg.Checked = true;

			comboBoxSection.DisplayMember = "Key";
			comboBoxSection.ValueMember = "Value";
			comboBoxSection.DataSource = new BindingSource(Enum.GetValues(typeof(SoundSection)).OfType<Enum>().ToDictionary(s => s.GetStringValue()), null);
			comboBoxSection.SelectedValue = SoundSection.Run;

			groupBoxPosition.Enabled = false;
			textBoxRadius.Enabled = false;
			buttonRemove.Enabled = buttonApply.Enabled = false;

			toolStripMenuItemError.Checked = toolStripMenuItemWarning.Checked = toolStripMenuItemInfo.Checked = true;
			toolStripMenuItemError.BackColor = toolStripMenuItemWarning.BackColor = toolStripMenuItemInfo.BackColor = SystemColors.GradientActiveCaption;
		...
		private void ButtonDirectDot_Click(object sender, EventArgs e)
		{
			...
			if (float.TryParse(textBoxDirectX.Text, out x) && float.TryParse(textBoxDirectY.Text, out y))
			{
				...
				if (exist)
				{
					switch (MessageBox.Show(GetInterfaceString("message", "vertex_exist"), GetInterfaceString("motor_sound", "direct_input", "dot"), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
					{
						case DialogResult.Yes:
							break;
						default:
		...
		private void PictureBoxDrawArea_MouseDown(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
				...
				if (editState.CurrentSimuState == EditState.SimulationState.Disable || editState.CurrentSimuState == EditState.SimulationState.Stopped)
				{
					if (editState.CurrentInputMode != EditState.InputMode.SoundIndex)
					{
		...
		{
			{
				...
				toolStripStatusLabelX.Text = string.Format("{0} {1} km/h", GetInterfaceString("motor_sound", "status", "xy", "velocity"), velocity.ToString("0.00", CultureInfo.InvariantCulture));

				switch (editState.CurrentInputMode)
				{
					case EditState.InputMode.Pitch:
						toolStripStatusLabelY.Text = string.Format("{0} {1} ", GetInterfaceString("motor_sound", "status", "xy", "pitch"), pitch.ToString("0.00", CultureInfo.InvariantCulture));
						break;
					case EditState.InputMode.Volume:
						toolStripStatusLabelY.Text = string.Format("{0} {1} ", GetInterfaceString("motor_sound", "status", "xy", "volume"), volume.ToString("0.00", CultureInfo.InvariantCulture));
						break;
				}
				...
				if (editState.CurrentInputMode != EditState.InputMode.Volume)
				{
					...
					if (newHoveredVertex != hoveredVertexPitch)
					{
						if (newHoveredVertex != null)
						{
							...
							builder.AppendFormat("{0} {1} km/h", GetInterfaceString("motor_sound", "vertex_info", "velocity"), newHoveredVertex.X.ToString("0.00", CultureInfo.InvariantCulture));
							builder.AppendLine();
							builder.AppendFormat("{0} {1}", GetInterfaceString("motor_sound", "vertex_info", "pitch"), newHoveredVertex.Y.ToString("0.00", CultureInfo.InvariantCulture));
							builder.AppendLine();
							builder.AppendFormat("{0} {1}", GetInterfaceString("motor_sound", "vertex_info", "sound_index"), area != null ? area.Index : -1);

							toolTipVertexPitch.ToolTipTitle = GetInterfaceString("motor_sound", "vertex_info", "name");
				...
				if (editState.CurrentInputMode != EditState.InputMode.Pitch)
				{
					...
					if (newHoveredVertex != hoveredVertexVolume)
					{
						if (newHoveredVertex != null)
						{
							...
							builder.AppendFormat("{0} {1} km/h", GetInterfaceString("motor_sound", "vertex_info", "velocity"), newHoveredVertex.X.ToString("0.00", CultureInfo.InvariantCulture));
							builder.AppendLine();
							builder.AppendFormat("{0} {1}", GetInterfaceString("motor_sound", "vertex_info", "volume"), newHoveredVertex.Y.ToString("0.00", CultureInfo.InvariantCulture));
							builder.AppendLine();
							builder.AppendFormat("{0} {1}", GetInterfaceString("motor_sound", "vertex_info", "sound_index"), area != null ? area.Index : -1);

							toolTipVertexVolume.ToolTipTitle = GetInterfaceString("motor_sound", "vertex_info", "name");
		...
		private void TabControlEditor_Selecting(object sender, TabControlCancelEventArgs e)
		{
			if (editState.CurrentSimuState == EditState.SimulationState.Started || editState.CurrentSimuState == EditState.SimulationState.Paused)
			{
				e.Cancel = true;
		...
		private void ListViewSound_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (listViewSound.SelectedItems.Count == 1)
			{
				if (listViewSound.SelectedItems[0] == listViewSound.FocusedItem)
				{
					ListViewItem item = listViewSound.FocusedItem;
					SoundCfg.Sound sound = (SoundCfg.Sound)item.Tag;
					SoundSection section = (SoundSection)item.Group.Tag;

					buttonAdd.Enabled = false;
					buttonRemove.Enabled = buttonApply.Enabled = true;

					comboBoxSection.SelectedValue = section;

					{
						if (item.SubItems[0].Tag is Enum)
						{
							Enum key = (Enum)item.SubItems[0].Tag;

							comboBoxKey.Enabled = true;
							comboBoxKey.DisplayMember = "Key";
							comboBoxKey.ValueMember = "Value";
							comboBoxKey.DataSource = new BindingSource(Enum.GetValues(key.GetType()).OfType<Enum>().ToDictionary(s => s.GetStringValue()), null);
							comboBoxKey.SelectedValue = key;

							numericUpDownKeyIndex.Enabled = false;
						}
						else if (item.SubItems[0].Tag is int)
						{
							int key = (int)item.SubItems[0].Tag;

							comboBoxKey.Enabled = false;
							comboBoxKey.DataSource = null;

							numericUpDownKeyIndex.Enabled = true;
							numericUpDownKeyIndex.Value = key;
						}
						else
						{
							comboBoxKey.Enabled = numericUpDownKeyIndex.Enabled = false;
						}
					}

					textBoxFileName.Text = sound.FileName;

					checkBoxPosition.Checked = groupBoxPosition.Enabled = sound.IsPositionDefined;
					textBoxPositionX.Text = sound.Position.X.ToString(CultureInfo.InvariantCulture);
					textBoxPositionY.Text = sound.Position.Y.ToString(CultureInfo.InvariantCulture);
					textBoxPositionZ.Text = sound.Position.Z.ToString(CultureInfo.InvariantCulture);

					checkBoxRadius.Checked = textBoxRadius.Enabled = sound.IsRadiusDefined;
					textBoxRadius.Text = sound.Radius.ToString(CultureInfo.InvariantCulture);
				}
				else
				{
		...
		{
			AddOrApplySound(true);
		}

		private void ComboBoxSection_SelectedIndexChanged(object sender, EventArgs e)
		{
			SoundSection section = (SoundSection)comboBoxSection.SelectedValue;
			...
			switch (section)
			{
				case SoundSection.Brake:
					type = typeof(BrakeKey);
					break;
				case SoundSection.Compressor:
					type = typeof(CompressorKey);
					break;
				case SoundSection.Suspension:
					type = typeof(SuspensionKey);
					break;
				...
				case SoundSection.Buzzer:
					type = typeof(BuzzerKey);
					break;
				case SoundSection.PilotLamp:
					type = typeof(PilotLampKey);
					break;
				case SoundSection.BrakeHandle:
					type = typeof(BrakeHandleKey);
					break;
				case SoundSection.MasterController:
					type = typeof(MasterControllerKey);
					break;
				case SoundSection.Reverser:
					type = typeof(ReverserKey);
					break;
				case SoundSection.Breaker:
					type = typeof(BreakerKey);
					break;
				case SoundSection.RequestStop:
					type = typeof(RequestStopKey);
					break;
				case SoundSection.Others:
					type = typeof(OthersKey);
					break;
			}
			...
			if (type != null)
			{
				...
				comboBoxKey.DisplayMember = "Key";
				comboBoxKey.ValueMember = "Value";
				comboBoxKey.DataSource = new BindingSource(Enum.GetValues(type).OfType<Enum>().ToDictionary(s => s.GetStringValue()), null);
		...
		{
			textBoxRadius.Enabled = checkBoxRadius.Checked;
		}

		private void CheckBoxPosition_CheckedChanged(object sender, EventArgs e)
		{
			groupBoxPosition.Enabled = checkBoxPosition.Checked;
		}

		private void RadioButtonOpenCfg_CheckedChanged(object sender, EventArgs e)
		{
			radioButtonOpenXml.Checked = !radioButtonOpenCfg.Checked;
		}

		private void RadioButtonOpenXml_CheckedChanged(object sender, EventArgs e)
		{
			radioButtonOpenCfg.Checked = !radioButtonOpenXml.Checked;
		}

		private void RadioButtonSaveCfg_CheckedChanged(object sender, EventArgs e)
		{
			if (isSoundXml && radioButtonSaveCfg.Checked)
			{
				switch (MessageBox.Show(GetInterfaceString("message", "cfg_save"), GetInterfaceString("sound_cfg", "save_setting", "format", "name"), MessageBoxButtons.OKCancel, MessageBoxIcon.Warning))
				{
					case DialogResult.Cancel:
						radioButtonSaveCfg.Checked = false;
						return;
				}
			}

			radioButtonSaveXml.Checked = !radioButtonSaveCfg.Checked;
		}

		private void RadioButtonSaveXml_CheckedChanged(object sender, EventArgs e)
		{
			radioButtonSaveCfg.Checked = !radioButtonSaveXml.Checked;
		}
		...
		private void ToolStripMenuItemError_Click(object sender, EventArgs e)
		{
			toolStripMenuItemError.Checked = !toolStripMenuItemError.Checked;

			toolStripMenuItemError.BackColor = toolStripMenuItemError.Checked ? SystemColors.GradientActiveCaption : SystemColors.Control;

			ChangeLogMessagesVisible(MessageType.Critical, toolStripMenuItemError.Checked);
			ChangeLogMessagesVisible(MessageType.Error, toolStripMenuItemError.Checked);
		}

		private void ToolStripMenuItemWarning_Click(object sender, EventArgs e)
		{
			toolStripMenuItemWarning.Checked = !toolStripMenuItemWarning.Checked;

			toolStripMenuItemWarning.BackColor = toolStripMenuItemWarning.Checked ? SystemColors.GradientActiveCaption : SystemColors.Control;

			ChangeLogMessagesVisible(MessageType.Warning, toolStripMenuItemWarning.Checked);
		}

		private void ToolStripMenuItemInfo_Click(object sender, EventArgs e)
		{
			toolStripMenuItemInfo.Checked = !toolStripMenuItemInfo.Checked;

			toolStripMenuItemInfo.BackColor = toolStripMenuItemInfo.Checked ? SystemColors.GradientActiveCaption : SystemColors.Control;

			ChangeLogMessagesVisible(MessageType.Information, toolStripMenuItemInfo.Checked);
		}
		...
		private void ButtonOpen_Click(object sender, EventArgs e)
		{
			...
			{
				switch (MessageBox.Show(GetInterfaceString("message", "not_yet_saved"), GetInterfaceString("sound_cfg", "entry_edit", "value", "open"), MessageBoxButtons.OKCancel, MessageBoxIcon.Information))
				{
					case DialogResult.Cancel:
						return;
				}
			...
			using (OpenFileDialog dialog = new OpenFileDialog())
			{
				...
				Uri basePath = new Uri(string.Format(@"{0}\", Path.GetDirectoryName(fileName)).Replace("%", "%25"));
				Uri filePath = new Uri(dialog.FileName.Replace("%", "%25"));

				string relativePath = basePath.MakeRelativeUri(filePath).ToString();

				textBoxFileName.Text = Uri.UnescapeDataString(relativePath).Replace("%25", "%");
			}
		}
	}
}
