	...
	internal static class Illustrations {
		...
		internal static Bitmap CreateRouteGradientProfile(int Width, int Height, bool inGame)
		{
			if (TrackManager.CurrentTrack.Elements.Length > 36 && Game.Stations.Length == 0)
			{
				...
				throw new Exception(OpenBveApi.Interface.Interface.GetInterfaceString("errors_route_corrupt_nostations"));
			}
			...
			// ROUTE MARKERS
			{
				...
				if (k == 0)
				{
					...
					throw new Exception(OpenBveApi.Interface.Interface.GetInterfaceString("errors_route_corrupt_withtrack"));
				}
