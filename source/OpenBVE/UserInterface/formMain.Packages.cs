...
namespace OpenBve
{
	internal partial class formMain
	{
		...
		private void RefreshPackages()
		{
			...
			{
				MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_database_save_error"));
			}
		...
		private void buttonInstall_Click(object sender, EventArgs e)
		{
			if (creatingPackage)
			{
				if (textBoxPackageName.Text == OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none"))
				{
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_creation_name"));
					return;
				}
				if (textBoxPackageAuthor.Text == OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none"))
				{
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_creation_author"));
					return;
				}
				//LINK: Doesn't need checking

				if (textBoxPackageDescription.Text == OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none"))
				{
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_creation_description"));
					return;
				}
				if (!Version.TryParse(textBoxPackageVersion.Text, out currentPackage.PackageVersion))
				{
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_creation_version"));
					return;
				}
			...
			else
			{
				...
				if (Reccomendations != null)
				{
					...
					labelDependancyErrorHeader.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_reccomends_unmet_header");
					labelMissingDependanciesText1.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_reccomends_unmet");
				...
				else
				{
					switch (Info)
					{
						case VersionInformation.NewerVersion:
							labelVersionError.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_version_new");
							labelCurrentVersionNumber.Text = oldPackage.PackageVersion.ToString();
							break;
						case VersionInformation.SameVersion:
							labelVersionError.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_version_same");
							labelCurrentVersionNumber.Text = currentPackage.PackageVersion.ToString();
							break;
						case VersionInformation.OlderVersion:
							labelVersionError.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_version_old");
							labelCurrentVersionNumber.Text = oldPackage.PackageVersion.ToString();
							break;
					}
		...
		private void buttonSelectPackage_Click(object sender, EventArgs e)
		{
			if (openPackageFileDialog.ShowDialog() == DialogResult.OK)
			{
				...
				if (currentPackage != null)
				{
					...
					{
						linkLabelPackageWebsite.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none_website");
					}
				...
				else
				{
					...
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_install_invalid"));
				}
			}
		}
		...
		private void Extract(Package packageToReplace = null)
		{
			...
			workerThread.RunWorkerCompleted += delegate
			{
				if (!ProblemEncountered)
				{
					...
					labelInstallSuccess1.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_success");
					labelInstallSuccess2.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_success_header");
					labelListFilesInstalled.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_success_files");
				}
		...
		private void OnWorkerReportsProblem(object sender, ProblemReport e)
		{
			...
			if (!creatingPackage)
			{
				labelInstallSuccess1.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_failure");
				labelInstallSuccess2.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_failure_header");
			}
			else
			{
				labelInstallSuccess1.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_failure");
				labelInstallSuccess2.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_failure_header");
			}
			labelListFilesInstalled.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_failure_error");
			buttonInstallFinish.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_success");
				...
				if (!simpleList)
				{
					if (isDependancy)
					{
						packageToAdd = new object[]
						{
							...
							OpenBveTranslate.Interface.GetInterfaceString("packages_dependancy"), packageList[i].GUID
						};
					}
					else if (isRecommendation)
					{
						packageToAdd = new object[]
						{
							packageList[i].Name, packageList[i].MinimumVersion, packageList[i].MaximumVersion,
							packageList[i].PackageType.ToString(),
							OpenBveTranslate.Interface.GetInterfaceString("packages_recommendation"), packageList[i].GUID
						};
					}
					else
					{
		...
		internal void UninstallPackage(Package packageToUninstall, ref List<Package> Packages)
		{
			...
			{
				PopulatePackageList(brokenDependancies, dataGridViewBrokenDependancies, false, false, false);
				labelMissingDependanciesText1.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_uninstall_broken");
				HidePanels();
				panelDependancyError.Show();
			}
			if (Manipulation.UninstallPackage(packageToUninstall, currentDatabaseFolder, ref uninstallResults))
			{
				...
				labelUninstallSuccess.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_uninstall_success");
				labelUninstallSuccessHeader.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_uninstall_success_header");
			...
			else
			{
				...
				if (uninstallResults == null)
				{
					...
					textBoxUninstallResult.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_uninstall_missing_xml");
		...
		private void buttonUninstallPackage_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none"));
			}
		}
		...
		private void buttonUninstallFinish_Click(object sender, EventArgs e)
		{
			if (currentPackage != null)
			{
				...
				{
					if (MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_uninstall_database_remove"), OpenBveTranslate.Interface.GetInterfaceString("program_title"), MessageBoxButtons.YesNo) == DialogResult.Yes)
					{
		...
		{
			currentPackage = null;
			labelInstallText.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_header");
			buttonBack2.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_button_cancel");
		...
		private void buttonDepends_Click(object sender, EventArgs e)
		{
			...
			{
				case PackageType.Route:
					if (ShowVersionDialog(ref dependantPackage.MinimumVersion, ref dependantPackage.MaximumVersion, dependantPackage.Version, OpenBveTranslate.Interface.GetInterfaceString("packages_creation_dependancies_add")) == DialogResult.OK)
					{
		...
		private void buttonReccomends_Click(object sender, EventArgs e)
		{
			...
			{
				case PackageType.Route:
					if (ShowVersionDialog(ref dependantPackage.MinimumVersion, ref dependantPackage.MaximumVersion, dependantPackage.Version, OpenBveTranslate.Interface.GetInterfaceString("packages_creation_reccomends_add")) == DialogResult.OK)
					{
		...
		private void buttonRemove_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (dataGridViewPackages3.Rows[row].Cells[dataGridViewPackages3.ColumnCount - 2].Value.ToString() == OpenBveTranslate.Interface.GetInterfaceString("packages_dependancy"))
				{
		...
		private void buttonCreatePackage_Click(object sender, EventArgs e)
		{
			...
			catch (Exception ex)
			{
				if (ex is DirectoryNotFoundException)
				{
					...
					if (ex.Message == string.Empty)
					{
						...
						MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_filename_empty"), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
					}
					else
					{
						//Directory doesn't exist
						MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_directory_missing") + ex.Message, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
					}
				...
				else if (ex is UnauthorizedAccessException)
				{
					...
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_directory_nowrite") + directory, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
				}
				else
				{
					//Generic error
					MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_file_generic") + currentPackage.FileName, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Hand);
				}
				return;
			}
			HidePanels();
			...
			workerThread.RunWorkerCompleted += delegate {
				if (ProblemEncountered == false)
				{
					...
					labelInstallSuccess1.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_success");
					labelInstallSuccess2.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_success_header");
					labelListFilesInstalled.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_success_files");
					buttonInstallFinish.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_success");
				}
				panelPleaseWait.Hide();
				panelSuccess.Show();
		...
		private void buttonCreateProceed_Click(object sender, EventArgs e)
		{
			...
			if (fi == null)
			{
				...
				MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_creation_invalid_filename"));
				return;
			}
			try
			{
			...
			buttonSelectPackage.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_proceed");
			...
			labelInstallText.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_header");
			textBoxPackageName.Text = currentPackage.Name;
		...
		private void Q1_CheckedChanged(object sender, EventArgs e)
		{
			...
			if (radioButtonQ1Yes.Checked == true)
			{
				if (Database.currentDatabase.InstalledRoutes.Count == 0 && Database.currentDatabase.InstalledTrains.Count == 0 && Database.currentDatabase.InstalledOther.Count == 0)
				{
					...
					string test = OpenBveTranslate.Interface.GetInterfaceString("packages_replace_noneavailable");
		...
		private static DialogResult ShowInputDialog(ref string input)
		{
			...
			Form inputBox = new Form
			{
				...
				ClientSize = size,
				Text = OpenBveTranslate.Interface.GetInterfaceString("packages_list_website")
			};
			...
			Button okButton = new Button
			{
				...
				Text = OpenBveTranslate.Interface.GetInterfaceString("packages_button_ok"),
			...
			Button cancelButton = new Button
			{
				...
				Name = "cancelButton",
				Size = new System.Drawing.Size(75, 23),
				Text = OpenBveTranslate.Interface.GetInterfaceString("packages_button_cancel"),
		...
		private static DialogResult ShowVersionDialog(ref Version minimumVersion, ref Version maximumVersion, string currentVersion, string label)
		{
			...
			Form inputBox = new Form
			{
				...
				StartPosition = FormStartPosition.CenterScreen,
				ClientSize = size,
				Text = OpenBveTranslate.Interface.GetInterfaceString("packages_list_version")
			};

			System.Windows.Forms.Label minLabel = new Label
			{
				Text = OpenBveTranslate.Interface.GetInterfaceString("packages_list_minimum"),
			...
			{
				Text = OpenBveTranslate.Interface.GetInterfaceString("packages_list_maximum"),
			...
			{
				MessageBox.Show(OpenBveTranslate.Interface.GetInterfaceString("packages_creation_version_invalid"));
			}
			return result;
		}
		...
		private void replacePackageButton_Click(object sender, EventArgs e)
		{
			...
			labelNewGUID.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_replace_id");
			textBoxGUID.Text = currentPackage.GUID;
			panelNewPackage.Enabled = true;
		...
		private void SaveFileNameButton_Click(object sender, EventArgs e)
		{
			...
			{
				Title = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_save"),
		...
		private void ResetInstallerPanels()
		{
			...
			//Reset text
			textBoxPackageAuthor.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none");
			textBoxPackageName.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none");
			textBoxPackageDescription.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none");
			textBoxPackageVersion.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none");
			buttonSelectPackage.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_install_select");
			labelNewGUID.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_creation_new_id");
			linkLabelPackageWebsite.Links.Clear();
			linkLabelPackageWebsite.Text = OpenBveTranslate.Interface.GetInterfaceString("packages_selection_none_website");
