	...
	internal static class NearestTrain
	{
		...
		private static TrainBase CreateDummyTrain()
		{
			...
			for (int i = 0; i < train.Cars.Length; i++)
			{
				...
				train.Cars[i].CarBrake.mainReservoir = new MainReservoir(Status.MainReservoirPressure * 1000);
				train.Cars[i].CarBrake.brakePipe = new BrakePipe(Status.BrakePipePressure * 1000);
				train.Cars[i].CarBrake.brakeCylinder = new BrakeCylinder(Status.BrakeCylinderPressure * 1000);
				train.Cars[i].CarBrake.straightAirPipe = new StraightAirPipe(Status.StraightAirPipePressure * 1000);
