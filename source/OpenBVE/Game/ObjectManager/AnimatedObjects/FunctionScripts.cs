						...
						if (Train != null)
						{
							...
							for (int j = 0; j < Train.Cars.Length; j++)
							{
								if (Train.Cars[j].Specs.IsMotorCar)
								{
									...
									{
										Function.Stack[s] = Train.Cars[j].Specs.CurrentAccelerationOutput * Math.Sign(Train.Cars[j].Specs.CurrentSpeed);
									}
					...
					case Instructions.TrainAccelerationMotorOfCar:
						if (Train != null)
						{
							...
							if (j >= 0 & j < Train.Cars.Length)
							{
								...
								if (Train.Cars[j].Specs.CurrentAccelerationOutput < 0.0)
								{
									Function.Stack[s - 1] = Train.Cars[j].Specs.CurrentAccelerationOutput * Math.Sign(Train.Cars[j].Specs.CurrentSpeed);
								}
								else if (Train.Cars[j].Specs.CurrentAccelerationOutput > 0.0)
								{
					...
					case Instructions.SectionAspectNumber:
						if (IsPartOfTrain)
						{
							...
							if (nextSectionIndex >= 0 & nextSectionIndex < CurrentRoute.Sections.Length)
							{
								...
								{
									Function.Stack[s] = CurrentRoute.Sections[nextSectionIndex].Aspects[a].Number;
								}
								else
								{
									Function.Stack[s] = 0;
						...
						else if (SectionIndex >= 0 & SectionIndex < CurrentRoute.Sections.Length)
						{
							...
							{
								Function.Stack[s] = CurrentRoute.Sections[SectionIndex].Aspects[a].Number;
							}
							else
							{
								Function.Stack[s] = 0;
