	...
	partial class Bve5ScenarioParser
	{
		...
		static void SetBackground(string key, string[] Arguments, ref RouteData Data, int BlockIndex, double[] UnitOfLength)
		{
			...
			if (BackgroundIdx == -1 && sttype != -1)
			{
				...
				{
					Data.Backgrounds[BackgroundIdx] = new Background(Arguments[0], new BackgroundManager.BackgroundObject((ObjectManager.StaticObject) Data.Structure.Objects[sttype]));
				}
				else
				{
					//The background object is null (presume the load has failed?, just chuck in a new static one for the minute)
					Data.Backgrounds[BackgroundIdx] = new Background(Arguments[0], new BackgroundManager.StaticBackground(null, 6, false));
				}
			}
			if (BackgroundIdx != -1)
			{
		...
		static void ChangeBrightness(bool Immediate, string[] Arguments, ref RouteData Data, int BlockIndex)
		{
			...
			if (NumberFormats.TryParseFloatVb6(Arguments[0], out b))
			{
				if (Immediate)
				{
					...
					Array.Resize<Brightness>(ref Data.Blocks[BlockIndex].Brightness, n + 2);
