						...
						case "coordinates":
							{
								...
								Vector2 parsedCoordinates = NumberFormats.ParseVector2(Arguments.Skip(1).ToArray(), Command, "Coordinates", i, FileName);

								if (j >= 0 & j < Builder.Vertices.Length)
								{
									Builder.Vertices[j].TextureCoordinates = parsedCoordinates;
								} else {
