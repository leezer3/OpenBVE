...
namespace OpenBve
{
	internal class RailDriver32Bit : Joystick, PIEDataHandler, PIEErrorHandler
	{
		internal RailDriver32Bit(int newHandle)
		{
			Handle = newHandle;
		...
		{
			for (int i = 0; i < JoystickManager.Win32PIEDevices.Length; i++)
			{
				if (JoystickManager.Win32PIEDevices[i] == sourceDevice)
				{
					//Source device found, so map it
					for (int j = 0; j < JoystickManager.AttachedJoysticks.Length; j++)
					{
						if (JoystickManager.AttachedJoysticks[j] is RailDriver32Bit && JoystickManager.AttachedJoysticks[j].Handle == i)
						{
							for (int r = 0; r < sourceDevice.ReadLength; r++)
							{
								JoystickManager.AttachedJoysticks[j].currentState[r] = data[r];
							}
						}
					}
				}
		...
		{
			JoystickManager.Win32PIEDevices[0].ReadData(ref currentState);
		}
		...
		internal override void SetDisplay(int speed)
		{
			...
			while (result == 404)
			{
				result = JoystickManager.Win32PIEDevices[Handle].WriteData(wData);
			}
