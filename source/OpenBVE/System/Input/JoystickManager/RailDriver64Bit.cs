...
namespace OpenBve
{
	internal class RailDriver64Bit : Joystick, PIEDataHandler, PIEErrorHandler
	{
		internal RailDriver64Bit(int newHandle)
		{
			Handle = newHandle;
		...
		{
			for (int i = 0; i < JoystickManager.Win64PIEDevices.Length; i++)
			{
				if (JoystickManager.Win64PIEDevices[i] == sourceDevice)
				{
					//Source device found, so map it
					for (int j = 0; j < JoystickManager.AttachedJoysticks.Length; j++)
					{
						if (JoystickManager.AttachedJoysticks[j] is RailDriver64Bit && JoystickManager.AttachedJoysticks[j].Handle == i)
						{
		...
		{
			JoystickManager.Win64PIEDevices[0].ReadData(ref currentState);
		}
		...
		internal override void SetDisplay(int speed)
		{
			...
			while (result == 404)
			{
				result = JoystickManager.Win64PIEDevices[Handle].WriteData(wData);
			}
