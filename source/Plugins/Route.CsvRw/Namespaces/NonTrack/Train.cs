			...
			switch (Command)
			{
				...
				case TrainCommand.DownloadLocation:
				{
					if (Arguments.Length > 0)
					{
						...
						if (Uri.TryCreate(Arguments[0], UriKind.Absolute, out Uri uriResult) && (uriResult.Scheme == Uri.UriSchemeHttp  || uriResult.Scheme == Uri.UriSchemeHttps) && uriResult.Host.Replace("www.", string.Empty).Split('.').Length > 1 && uriResult.HostNameType == UriHostNameType.Dns && uriResult.Host.Length > uriResult.Host.LastIndexOf(".", StringComparison.InvariantCulture) + 1 && 100 >= Arguments[0].Length)
						{
