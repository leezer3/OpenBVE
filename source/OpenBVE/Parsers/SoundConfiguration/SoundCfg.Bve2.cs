...
namespace OpenBve
{
	class BVE2SoundParser
	{
		...
		internal static void Parse(string trainFolder, TrainManager.Train train)
		{
			...
			train.Cars[train.DriverCar].CarBrake.Release = new CarSound(Program.Sounds.RegisterBuffer(OpenBveApi.Path.CombineFile(trainFolder, "Brake.wav"), SoundCfgParser.smallRadius), center);
