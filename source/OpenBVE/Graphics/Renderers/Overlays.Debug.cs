...
namespace OpenBve.Graphics.Renderers
{
	internal partial class Overlays
	{
		...
		private void RenderDebugOverlays()
		{
			...
			{
				if (TrainManager.PlayerTrain.Cars[i].Engine.ProvidesPower)
				{
					car = i;
					break;
				}
			...
			string[] Lines = {
				...
				"power (car " + car.ToString(Culture) +  "): " + (TrainManager.PlayerTrain.Cars[car].Engine.CurrentAcceleration < 0.0 ? TrainManager.PlayerTrain.Cars[car].Engine.CurrentAcceleration * Math.Sign(TrainManager.PlayerTrain.Cars[car].CurrentSpeed) : TrainManager.PlayerTrain.Cars[car].Engine.CurrentAcceleration * (double)TrainManager.PlayerTrain.Handles.Reverser.Actual).ToString("0.0000", Culture) + " m/s²",
