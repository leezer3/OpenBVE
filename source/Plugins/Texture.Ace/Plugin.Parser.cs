...
namespace Texture.Ace {
	public partial class Plugin
	{
		...
		{
			ulong identifier = LittleEndianBinaryExtensions.ToUInt64(data, 0);
		...
		{
			byte[] bytes = File.ReadAllBytes(file);
			ulong identifier = LittleEndianBinaryExtensions.ToUInt64(bytes, 0);
		...
		private static void QueryDimensionsFromUncompressedData(byte[] data, out int width, out int height)
		{
			...
			int unknown1 = ToInt32(data, 16);
			if (unknown1 != 1)
			{
				throw new InvalidDataException();
			}
			// second unknown
			width = ToInt32(data, 24);
			height = ToInt32(data, 28);
		...
		private static OpenBveApi.Textures.Texture LoadFromUncompressedData(byte[] data)
		{
			using (MemoryStream stream = new MemoryStream(data))
			{
				using (BinaryReader reader = new BinaryReader(stream))
				{
					...
					{
						throw new InvalidDataException("Unsupported ACE compression type.");
					}
					...
					{
						throw new InvalidDataException("Invalid number of ACE channels.");
					}
