						...
						if (a > d & b < c)
						{
							if (a > c)
							{
								...
								if (Cars[k].CurrentSpeed < Trains[j].Cars[0].CurrentSpeed)
								{
									...
									{
										a = Cars[h + 1].FrontAxle.Follower.TrackPosition -
											Cars[h + 1].FrontAxle.Position + 0.5 * Cars[h + 1].Length;
										b = Cars[h].RearAxle.Follower.TrackPosition -
											Cars[h].RearAxle.Position - 0.5 * Cars[h].Length;
							...
							else
							{
								...
								if (Cars[0].CurrentSpeed > Trains[j].Cars[k].CurrentSpeed)
								{
									...
									{
										a = Cars[h - 1].RearAxle.Follower.TrackPosition -
											Cars[h - 1].RearAxle.Position - 0.5 * Cars[h - 1].Length;
										b = Cars[h].FrontAxle.Follower.TrackPosition -
											Cars[h].FrontAxle.Position + 0.5 * Cars[h].Length;
			...
			for (int j = 0; j < bufferTrackPositions.Count; j++)
			{
				...
				if (bPa > bufferTrackPositions[j].TrackPosition & bPb < bufferTrackPositions[j].TrackPosition)
				{
					...
					if (da < db)
					{
						...
						if (TrainManagerBase.CurrentOptions.Derailments &&
							Math.Abs(Cars[0].CurrentSpeed) > CriticalCollisionSpeedDifference)
						{
						...
						for (int h = 1; h < Cars.Length; h++)
						{
							bPa = Cars[h - 1].RearAxle.Follower.TrackPosition -
								  Cars[h - 1].RearAxle.Position - 0.5 * Cars[h - 1].Length;
							bPb = Cars[h].FrontAxle.Follower.TrackPosition -
								Cars[h].FrontAxle.Position + 0.5 * Cars[h].Length;
							...
							if (d < 0.0)
							{
								...
								if (TrainManagerBase.CurrentOptions.Derailments &&
									Math.Abs(Cars[h].CurrentSpeed) >
									CriticalCollisionSpeedDifference)
								{
									Derail(h, timeElapsed);
								}
					...
					else
					{
						...
						if (TrainManagerBase.CurrentOptions.Derailments &&
							Math.Abs(Cars[c].CurrentSpeed) > CriticalCollisionSpeedDifference)
						{
						...
						for (int h = Cars.Length - 2; h >= 0; h--)
						{
							bPa = Cars[h + 1].FrontAxle.Follower.TrackPosition -
								Cars[h + 1].FrontAxle.Position + 0.5 * Cars[h + 1].Length;
							bPb = Cars[h].RearAxle.Follower.TrackPosition -
								  Cars[h].RearAxle.Position - 0.5 * Cars[h].Length;
