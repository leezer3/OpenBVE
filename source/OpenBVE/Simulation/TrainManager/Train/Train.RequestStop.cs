			...
			public override void RequestStop(RequestStop stopRequest)
			{
				...
				if (Program.RandomNumberGenerator.Next(0, 100) <= stopRequest.Probability)
				{
					...
					else
					{
						...
						{
							Game.AddMessage(stopRequest.PassMessage, MessageDependency.None, GameMode.Normal, MessageColor.White, Game.SecondsSinceMidnight + 10.0, null);
						}
					...
					{
						Game.AddMessage(stopRequest.StopMessage, MessageDependency.None, GameMode.Normal, MessageColor.White, Game.SecondsSinceMidnight + 10.0, null);
					}
				}
				else
				{
