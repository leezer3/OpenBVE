...
namespace TrackManager
{
	public struct TrackFollower
	{
		...
		{
			Update(CurrentTrack, this.TrackPosition, true, AddTrackInaccuracy);
		}
		...
		public void Update(Track CurrentTrack, double NewTrackPosition, bool UpdateWorldCoordinates, bool AddTrackInaccurary)
		{
			...
			while (i >= 0 && NewTrackPosition < CurrentTrack.Elements[i].StartingTrackPosition)
			{
				...
				double tb = -0.01;
				CheckEvents(CurrentTrack, i, -1, ta, tb);
				i--;
			}
			...
			// events
			CheckEvents(CurrentTrack, i, Math.Sign(db - da), da, db);
			//Update the odometer
		...
		private void CheckEvents(Track CurrentTrack, int ElementIndex, int Direction, double OldDelta, double NewDelta)
		{
