...
namespace TrainManager.Car
{
	...
	public class BveReAdhesionDevice : AbstractReAdhesionDevice
	{
		...
		{
			if (TrainManagerBase.currentHost.InGameTime < NextUpdateTime || Car.Engine.MaximumCurrentAcceleration == -1)
			{
				return;
			}
			...
			{
				MaximumAccelerationOutput = Car.Engine.MaximumCurrentAcceleration * this.ApplicationFactor;
				TimeStable = 0.0;
			}
			else
			{
				...
				if (TimeStable >= this.ReleaseInterval)
				{
					...
					if (this.ReleaseFactor != 0.0 & MaximumAccelerationOutput <= Car.Engine.MaximumCurrentAcceleration + 1.0)
					{
