...
namespace OpenBveShared
{
	public class AnimatedWorldObject : WorldObject
	{
		...
		public override void Update(double TimeElapsed, bool ForceUpdate)
		{
			...
			double ta = World.CameraTrackFollower.TrackPosition + World.CameraCurrentAlignment.Position.Z - Renderer.BackgroundImageDistance - World.ExtraViewingDistance;
			double tb = World.CameraTrackFollower.TrackPosition + World.CameraCurrentAlignment.Position.Z + Renderer.BackgroundImageDistance + World.ExtraViewingDistance;
			...
			if (visible | ForceUpdate)
			{
				...
				if (!Visible)
				{
					Renderer.ShowObject(Object.ObjectIndex, ObjectType.Dynamic, Interface.CurrentOptions.TransparencyMode, World.CameraRestriction != Camera.RestrictionMode.NotAvailable);
					Visible = true;
				}
			}
			else
