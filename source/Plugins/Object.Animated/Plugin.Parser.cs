								...
								for (int j = 0; j < objCount; j++)
								{
									if (obj[j] != null)
									{
										if (obj[j] is StaticObject)
										{
											...
											{
												Prototype = s,
												Translation = OpenTK.Matrix4.CreateTranslation((float)position.X, (float)position.Y, (float)-position.Z)
											};
										...
										else if (obj[j] is AnimatedObjectCollection)
										{
											...
											for (int k = 0; k < a.Objects.Length; k++)
											{
												...
												{
													a.Objects[k].States[h].Translation *= OpenTK.Matrix4.CreateTranslation((float)position.X, (float)position.Y, (float)-position.Z);
												}
						...
						case "[object]":
							{
								...
								if (StateFiles != null)
								{
									...
									for (int j = 0; j < Result.Objects[ObjectCount].States.Length; j++)
									{
										...
										//Apply position
										Result.Objects[ObjectCount].States[j].Translation = OpenTK.Matrix4.CreateTranslation((float)Position.X, (float)Position.Y, (float)-Position.Z);
